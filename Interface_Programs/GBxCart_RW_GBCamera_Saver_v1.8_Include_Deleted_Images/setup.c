...
// Read the config-folder.ini file for folder
void read_config_folder(void) {
	FILE* configFolderfile = fopen ("config-folder.ini" , "rt");
	if (configFolderfile != NULL) {
		if (fscanf(configFolderfile, "%s", currentFolder) != 1) {
			fprintf(stderr, "Config folder file is corrupt\n");
		}
		fclose(configFolderfile);
	}
}
...
// Check if the logo check is correct
uint8_t logo_check (void) {
	currAddr = 0x0000;
	endAddr = 0x0180;
...
void save_to_bmp (char* filename) {
