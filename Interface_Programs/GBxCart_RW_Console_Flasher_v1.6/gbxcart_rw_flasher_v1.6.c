...
int main(int argc, char **argv) {
	...
	printf("GBxCart RW Flasher v1.6 by insideGadgets\n");
	printf("########################################\n");
	...
	if (argc >= 2) {
		...
		// ****** GB Flash Carts ******
		if (flashCartType == 1) {
			printf("32 KByte Gameboy Flash Cart (Audio as WE)\n");
			currAddr = 0x0000;
			endAddr = 0x7FFF;
		...
		if (flashCartType == 2) {
			printf("32 KByte Gameboy Flash Cart (WR as WE)\n");
			...
			if (gbxcartFirmwareVersion <= 7) {
				fclose(romFile);
				printf("Firmware R8 or higher is required for this functionality.\n");
				read_one_letter();
				return 1;
			}
		...
		printf("\nPlease select a Flash Cart:\n"\
					 "--- Gameboy ---\n"\
					 "1. 32 KByte (Audio as WE)\n"\
					 "2. 32 KByte (WR as WE)\n"\
					 "3. 2 MByte (BV5)\n"\
					 "4. 1 MByte (ES29LV160)\n"\
					 "5. 1 MByte (AM29LV160DB / 29LV160CTTC)\n\n"\
					 "--- Gameboy Advance ---\n"\
					 "6. 16 MByte (MSP55LV128)\n"\
					 "7. 16 MByte (MSP55LV128M / 29GL128EHMC / 29LV128DTMC / 256M29EWH)\n\n"\
					 "x. Exit\n>");
		...
		if (optionSelected == '1') {
			...
			write_flash_config(6);
		}
		else if (optionSelected == '7') {
			write_flash_config(7);
		}
