...
char comports[RS232_PORTNR][100]={"/dev/ttyS0","/dev/ttyS1","/dev/ttyS2","/dev/ttyS3","/dev/ttyS4","/dev/ttyS5",
                       ...
                       "/dev/tty.wchusbserial630",
							  ...
							  "/dev/tty.wchusbserialfa110","/dev/tty.wchusbserialfa120","/dev/tty.wchusbserialfa130",
							  "/dev/tty.wchusbserialfa140","/dev/tty.wchusbserialfa150","/dev/tty.wchusbserialfa160",
};
...
int RS232_OpenComport(int comport_number, int baudrate, const char *mode)
{
  ...
  if((comport_number>=RS232_PORTNR)||(comport_number<0))
  {
	...
	// Detect the com port
	if (comport_number == 0) {
		char *portCommand = "ls /dev/ttyUSB* > comport.ini";
		system(portCommand);
		
		int portFound = 0;
		char portName[200];
		FILE* portFile = fopen ("comport.ini" , "rb");
		if (portFile != NULL) {
			if (fscanf(portFile, "%s", portName) == 1) {
				strncpy(comports[0], portName, 50);
				printf("Found %s\n", portName);
				portFound = 1;
			}
			fclose(portFile);
		...
		if (portFound == 0) {
			char *portCommand = "ls /dev/tty.wchusbserial* > comport.ini";
			system(portCommand);
		...
		if (portFound == 0) {
			char *portCommand = "ls /dev/tty.usbserial* > comport.ini";
			system(portCommand);
...
int RS232_OpenComport(int comport_number, int baudrate, const char *mode)
{
  ...
  switch(baudrate)
  {
    ...
    case 1000000 : strcpy(mode_str, "baud=1000000");
                   break;
	 case 1700000 : strcpy(mode_str, "baud=1700000");
                   break;
    default      : printf("invalid baudrate\n");
                   return(1);
                   break;
