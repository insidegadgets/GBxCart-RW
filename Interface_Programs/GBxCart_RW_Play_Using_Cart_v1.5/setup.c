...
char romsDirectory[255];
char emulatorDirectory[255];
char savesDirectory[255];
...
void read_play_config(void) {
	FILE* playconfigfile = fopen ("config-play.ini" , "rt");
	if (playconfigfile != NULL) {
		if (fscanf(playconfigfile, "%s\n%s\n%s", romsDirectory, savesDirectory, emulatorDirectory) != 3) {
			fprintf(stderr, "Play config file is corrupt\n");
		}
		fclose(playconfigfile);
	}
	else {
		fprintf(stderr, "Play config file not found\n");
	}
}
...
void read_gba_header (void) {
	...
	if (logoCheck == 1) {
		...
		printf ("Failed to read Nintendo logo. Please re-seat the cart.");
		read_one_letter();
		exit(1);
	}
}
