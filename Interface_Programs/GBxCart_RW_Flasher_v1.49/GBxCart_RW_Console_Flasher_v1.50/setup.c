...
uint8_t flashCartList[] = { 1, 29, 30, 31, 2, 42, 3, 4, 35, 5, 6, // GB iG carts
									  0, 52, 53, 8, 32, 9, 33, 10, 11, 12, 34, 13, 14, 15, 39, 40, 38, 56, 16, 17,  // GB carts
									  20, 27, 41, 43, 55, 21, 22, 23, 24, 25, 26, 36, 37, 54, // GBA carts
	...
	for (uint8_t x = 0; x <= RS232_PORTNR; x++) {
		if (RS232_OpenComport(x, bdrate, "8N1") == 0) { // Port opened
			...
			if (cartridgeMode == GB_MODE || cartridgeMode == GBA_MODE) {
				...
				if (RS232_OpenComport(cport_nr, 1700000, "8N1") == 0) { // Port opened
					set_mode('0');
					uint8_t cartridgeMode = request_value(CART_MODE);
...
// Read 1-1024 bytes from the file (or buffer) and write it the COM port with the command given
...
void gb_check_change_flash_id (uint8_t flashMethod) {
	...
	// Exit
	if (flashMethod == GB_FLASH_PROGRAM_7AAA_BIT01_SWAPPED) {
