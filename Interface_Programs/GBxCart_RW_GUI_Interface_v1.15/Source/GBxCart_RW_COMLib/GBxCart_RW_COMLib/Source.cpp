...
// GB WR Flash Cart - Write address and byte to flash
...
// GB WR Flash Cart - Sector erase
void gb_wr_flash_sector_erase(uint8_t sector) {
	gb_wr_flash_write_address_byte(0x555, 0xAA);
	gb_wr_flash_write_address_byte(0x2AA, 0x55);
	gb_wr_flash_write_address_byte(0x555, 0x80);
	gb_wr_flash_write_address_byte(0x555, 0xAA);
	gb_wr_flash_write_address_byte(0x2AA, 0x55);
	gb_wr_flash_write_address_byte(sector << 14, 0x30);
}
...
__declspec(dllexport) int erase_rom(uint8_t cartType, uint32_t &progress, uint8_t &cancelOperation) {
	...
	if (cartType == 2) { // 2 MByte (BV5) GB Flash Cart 
		...
		// Chip erase
		gb_wr_flash_write_address_byte(0xAAA, 0xAA);
		gb_wr_flash_write_address_byte(0x555, 0x55);
		gb_wr_flash_write_address_byte(0xAAA, 0x80);
		gb_wr_flash_write_address_byte(0xAAA, 0xAA);
		gb_wr_flash_write_address_byte(0x555, 0x55);
		gb_wr_flash_write_address_byte(0xAAA, 0x10);
	}
...
__declspec(dllexport) int write_rom(char* fileName, uint8_t flashCartType, uint32_t fileSize, uint32_t &progress, uint8_t &cancelOperation) {
	...
	// 32K GB Flash Cart (Normal WR)
		...
		if (gbxcartFirmwareVersion >= 4) {
			...
			while (currAddr <= endAddr) {
				if (currAddr % 0x4000 == 0) { // Erase sectors
					gb_wr_flash_sector_erase(sector);
					wait_for_flash_ff(currAddr);
					sector++;
				...
				set_mode(GB_FLASH_WRITE_64BYTE);
				com_write_bytes_from_file(GB_WR_FLASH_WRITE, romFile, 64);
				com_wait_for_ack();
				currAddr += 64;
				readBytes += 64;
	...
	// 2 MByte (AM29LV160DB / 29LV160CTTC) GB Flash Cart
		...
		for (uint16_t bank = 1; bank < romBanks; bank++) {
			...
			while (currAddr < endAddr) {
				...
				set_mode(GB_FLASH_WRITE_64BYTE);
				com_write_bytes_from_file(GB_WR_FLASH_BYTE_WRITE, romFile, 64);
				com_wait_for_ack();
				currAddr += 64;
				readBytes += 64;
	...
	// MSP55LV128M / 29GL128EHMC / 29LV128DTMC/ 256M29EWH 
