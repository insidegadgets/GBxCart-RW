...
int main(int argc, char **argv) {
	...
	printf("GBxCart RW Flasher v1.43 by insideGadgets\n");
	printf("#########################################\n");
	...
	if (argc >= 2) {
		...
		// ****** GBA Flash Carts ******
			...
			while (currAddr < endAddr) {
				...
				if (com_write_bytes_from_file_skip_FFs(GBA_FLASH_WRITE_BUFFERED_256BYTE, romFile, 256)) {
					com_wait_for_ack();
					currAddr += 256;
					readBytes += 256;
				}
		...
		if (optionSelected == 0) {
			printf("\n15. 512 KByte (SST39SF040)\n"\
					 ...
					 "34. insideGadgets 32MB, RTC 1Mbit Flash Save Flash Cart\n");
